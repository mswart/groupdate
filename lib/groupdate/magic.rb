...
module Groupdate
  class Magic
    ...
    def perform(relation, method, *args, &block)
      ...
      count =
        begin
          Hash[relation.send(method, :all, *args, &block).map { |k, v| [multiple_groups ? k[0...@group_index] + [cast_method.call(k[@group_index])] + k[(@group_index + 1)..-1] : cast_method.call(k), v] }]
